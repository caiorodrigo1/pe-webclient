...
@Component({
  selector: 'top-template-despacho',
  templateUrl: './template-despacho.component.html',
})
export class TemplateDespachoComponent implements OnInit, OnDestroy {
  entidade: string = 'Templates de Despacho';
  ...
  selecionados: ITemplateDespacho[] = [];
  ...
  protected carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.templateDespachoService
  ...
  protected confirmarExclusao(templateDespacho: TemplateDespacho): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${templateDespacho.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(templateDespacho.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.templateDespachoService
      .excluirTemplateDespacho(id)
      .subscribe({
        complete: () => {

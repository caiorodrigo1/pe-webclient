...
@Component({
  selector: 'top-template-processo',
  templateUrl: './template-processo.component.html',
})
export class TemplateProcessoComponent implements OnInit, OnDestroy {
  entidade: string = 'Templates de Processo';
  ...
  selecionados: ITemplateProcesso[] = [];
  ...
  protected carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.templateProcessoService
  ...
  protected confirmarExclusao(templateProcesso: TemplateProcesso): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${templateProcesso.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(templateProcesso.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.templateProcessoService
      .excluirTemplateProcesso(id)
      .subscribe({
        complete: () => {

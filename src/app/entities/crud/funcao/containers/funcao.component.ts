...
export class FuncaoComponent implements OnInit, OnDestroy {
  entidade: string = 'Funções';
  ...
  private carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.funcaoService.consultarFuncoes().subscribe({
      next: (resposta: HttpResponse<IEntidade>) => {
        const corpo = resposta.body;
        this.funcoes = corpo!.data;
  ...
  protected confirmarExclusao(funcao: Funcao): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${funcao.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(funcao.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.funcaoService.excluirFuncao(id).subscribe({
      complete: () => {

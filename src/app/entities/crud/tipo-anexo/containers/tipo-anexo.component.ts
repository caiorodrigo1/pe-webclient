...
@Component({
  selector: 'top-tipo-anexo',
  templateUrl: './tipo-anexo.component.html',
})
export class TipoAnexoComponent implements OnInit, OnDestroy {
  entidade: string = 'Tipos de Anexo';
  recipiente: string = 'Tipo de Anexo';
  tipoAnexos: ITipoAnexo[] = [];
  ...
  private carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.tipoAnexoService
      .consultarTipoAnexos()
      .subscribe({
        next: (resposta: HttpResponse<IEntidade>) => {
          const corpo = resposta.body;
          this.tipoAnexos = corpo!.data;
  ...
  protected confirmarExclusao(tipoAnexo: ITipoAnexo): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${tipoAnexo.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(tipoAnexo.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.tipoAnexoService
      .excluirTipoAnexo(id)
      .subscribe({
        complete: () => {

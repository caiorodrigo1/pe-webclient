...
@Component({
  selector: 'top-situacao-processo',
  templateUrl: './situacao-processo.component.html',
})
export class SituacaoProcessoComponent implements OnInit, OnDestroy {
  entidade: string = 'Situação dos Processos';
  recipiente: string = 'Situação do Processo';
  situacaoProcessos: ISituacaoProcesso[] = [];
  ...
  selecionados: ISituacaoProcesso[] = [];
  ...
  protected carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.situacaoProcessoService
      .consultarSituacaoProcessos()
      .subscribe({
        next: (resposta: HttpResponse<IEntidade>) => {
          const corpo = resposta.body;
          this.situacaoProcessos = corpo!.data;
          this.carregando = false;
        },
        error: (resposta: HttpErrorResponse) => {
          this.situacaoProcessos = [];
  ...
  protected confirmarExclusao(situacaoProcesso: SituacaoProcesso): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${situacaoProcesso.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(situacaoProcesso.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.situacaoProcessoService
      .excluirSituacaoProcesso(id)
      .subscribe({
        complete: () => {

...
@Component({
  selector: 'top-tipo-documento-atualizar',
  templateUrl: './tipo-documento-atualizar.component.html',
})
export class TipoDocumentoAtualizarComponent implements OnInit, OnDestroy {
  recipiente: string = 'Tipo de Documento';
  acao: string = 'Criar';
  ...
  private buscarRota(): void {
    this.activatedRoute.paramMap.subscribe((valor) => {
      ...
      if (id) {
        this.acao = 'Editar';
        this.consultarTipoDocumento$(Number(id));
      } else {
  ...
  private consultarTipoDocumento$(id: number): void {
    this.inscricaoConsulta = this.tipoDocumentoService
      .consultarTipoDocumento(id)
      .subscribe({
        next: (resposta: HttpResponse<IEntidade>) => {
          const corpo = resposta.body || { data: {} };
          this.tipoDocumento = corpo.data;
  ...
  protected salvar(): void {
    if (this.editForm.valid) {
      ...
      const tipoDocumento = this.criarFormulario();
      if (tipoDocumento.id === undefined) {
        this.chamarServico(
          this.tipoDocumentoService.incluirTipoDocumento(tipoDocumento),
          'criado'
        );
      } else {
        this.chamarServico(
          this.tipoDocumentoService.atualizarTipoDocumento(tipoDocumento),
          'atualizado'
        );
      }
    }
  ...
  private criarFormulario(): ITipoDocumento {
    return {
      ...new TipoDocumento(),

...
export class CargoComponent implements OnInit, OnDestroy {
  ...
  private carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.cargoService.consultarCargos().subscribe({
      next: (resposta: HttpResponse<IEntidade>) => {
        const corpo = resposta.body;
        this.cargos = corpo!.data;
  ...
  protected confirmarExclusao(cargo: Cargo): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${cargo.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(cargo.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.cargoService.excluirCargo(id).subscribe({
      complete: () => {

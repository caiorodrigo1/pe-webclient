...
@Component({
  selector: 'top-interessadoProcesso',
  templateUrl: './interessado-processo.component.html',
})
export class InteressadoProcessoComponent implements OnInit, OnDestroy {
  entidade: string = 'Interessados no Processo';
  recipiente: string = 'Interessado no Processo';
  interessadosProcesso: IInteressadoProcesso[] = [];
  ...
  selecionados: IInteressadoProcesso[] = [];
  ...
  protected carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.interessadoProcessoService
      .consultarInteressadosProcesso()
      .subscribe({
        next: (resposta: HttpResponse<IEntidade>) => {
          const corpo = resposta.body;
          this.interessadosProcesso = corpo!.data;
          this.carregando = false;
        },
        error: (resposta: HttpErrorResponse) => {
          this.interessadosProcesso = [];
  ...
  protected confirmarExclusao(interessadoProcesso: InteressadoProcesso): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${interessadoProcesso.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(interessadoProcesso.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.interessadoProcessoService
      .excluirInteressadoProcesso(id)
      .subscribe({
        complete: () => {

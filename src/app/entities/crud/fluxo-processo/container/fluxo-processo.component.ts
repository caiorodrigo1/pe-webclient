...
@Component({
  selector: 'top-fluxo-processo',
  templateUrl: './fluxo-processo.component.html',
})
export class FluxoProcessoComponent implements OnInit, OnDestroy {
  entidade: string = 'Fluxos de Processos';
  recipiente: string = 'Fluxo de Processo';
  fluxos: IFluxoProcesso[] = [];
  ...
  private carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.fluxoProcessoService
      .consultarFluxos()
      .subscribe({
        next: (resposta: HttpResponse<IEntidade>) => {
          const corpo = resposta.body || { data: [] };
          this.fluxos = corpo.data;
  ...
  protected confirmarExclusao(fluxo: IFluxoProcesso): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${fluxo.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(fluxo.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.fluxoProcessoService
      .excluirFluxoProcesso(id)
      .subscribe({
        complete: () => {

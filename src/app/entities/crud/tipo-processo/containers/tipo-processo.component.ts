...
export class TipoProcessoComponent implements OnInit, OnDestroy {
  ...
  private carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.tipoProcessoService
  ...
  protected confirmarExclusao(tipoProcesso: ITipoProcesso): void {
    this.confirmationService.confirm({
      message: `Tem certeza que deseja excluir ${tipoProcesso.nome}?`,
      header: `Excluir ${this.recipiente}`,
      icon: 'pi pi-exclamation-triangle',
      accept: () => this.excluir(tipoProcesso.id!),
    });
  }
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.tipoProcessoService
      .excluirTipoProcesso(id)
      .subscribe({
        complete: () => {

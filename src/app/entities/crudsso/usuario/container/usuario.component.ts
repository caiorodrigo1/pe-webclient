...
@Component({
  selector: 'top-usuario',
  templateUrl: './usuario.component.html',
})
export class UsuarioComponent implements OnInit, OnDestroy {
  entidade: string = 'Usuarios';
  recipiente: string = 'Usuario';
  usuarios: IUsuario[] = [];
  ...
  private carregarTabela$(): void {
    ...
    this.inscricaoTabela = this.usuarioService.consultarUsuarios().subscribe({
  ...
  protected confirmarExclusao(usuario: Usuario): void {
  ...
  private excluir(id: number): void {
    ...
    this.inscricaoExcluir = this.usuarioService.excluirUsuario(id).subscribe({
      complete: () => {
  ...
  protected onBlur(id: number): void {
    this.inscricaoConsultar = this.usuarioService
      .consultarUsuario(id)
      .subscribe({
        next: (resposta: HttpResponse<IEntidade>) => {
          const usuario: IUsuario = resposta.body?.data;
          this.mudarAtividade(usuario);
        },
        error: (resposta: HttpErrorResponse) => {
  ...
  private mudarAtividade(usuario: IUsuario): void {
    usuario.ativo = !usuario.ativo;
    usuario.clienteId = this.clienteId;
    delete usuario.identificador;
    this.inscricaoAtualizar = this.usuarioService
